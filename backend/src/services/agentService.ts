...
- After creating: "Property created and enriched! It's in 'new' stage. Next: Score against buy boxes. Should I do that now?"
...
- "Add with contacts and get status" → create_property_with_contacts → get_deal_status
    ...
    return [
      ...
      tool(
        ...
        },
        {
          ...
          schema: z.object({
            ...
            triggerType: z.enum(['deal_received', 'deal_enriched', 'deal_analyzed', 'deal_scored', 'deal_matched', 'score_threshold', 'deal_updated', 'webhook_received', 'schedule', 'manual']).describe('The event that triggers this automation'),
            ...
            actions: z.array(z.object({
              type: z.enum(['send_email', 'send_webhook', 'update_deal', 'add_tag', 'send_notification', 'log_event', 'score_against_buybox', 'enrich_deal', 'submit_to_marketplace']),
      ...
      tool(
        async ({ propertyAddress, propertyId }) => {
          try {
            const { DealPipeline, DocuSealSubmission } = await import('../models');
            ...
            // StateDocumentTemplate model has been removed. Document requirements are no longer tracked via templates.
            ...
            if (currentStage === 'new') {
              response.nextSteps.push('Score against buy boxes');
            }
        ...
        },
        {
          ...
          description: 'Get comprehensive deal status including: pipeline stage, required documents, blocking gates, and next steps. Use when user asks "what phase is this in", "what\'s needed", "what\'s blocking this deal", or "deal status".',
          schema: z.object({
